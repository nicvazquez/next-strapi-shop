 ...
 */
export async function getFilteredProducts(field: string, operator: string, value: string) {
    // Example: ?filters[category][$eqi]=management
    const response = await fetch(`http://127.0.0.1:1337/api/products?filters[${field}][$${operator}]=${value}`)
